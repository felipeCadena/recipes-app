...
describe('Verifica a renderização do botão de pesquisa', () => {
  it('Deve renderizar o botão de pesquisa nas rotas /meals e /drinks', () => {
    renderWithRouter(<App />, { route: '/meals' });
    const searchIconsMeals = screen.queryAllByTestId(SEARCH_TOP_BTN_TEST_ID);
    expect(searchIconsMeals).not.toHaveLength(0);

    renderWithRouter(<App />, { route: '/drinks' });
    const searchIconsDrinks = screen.queryAllByTestId(SEARCH_TOP_BTN_TEST_ID);
    expect(searchIconsDrinks).not.toHaveLength(0);
  });
...
describe('Teste de renderização condicional no Header', () => {
  it('Não deve renderizar o botão de pesquisa quando showSearchIcon é falso', () => {
    renderWithRouter(<Header showSearchIcon={ false } />);
